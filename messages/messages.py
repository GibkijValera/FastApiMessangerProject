...
from messages.attachments import MAX_FILE_SIZE, MAX_TOTAL_SIZE, ALLOWED_CONTENT_TYPES
...
async def patch_message(schema: PatchMessageSchema, user_id: int = Depends(get_current_user),
...
async def delete_message(schema: DeleteMessageSchema, user_id: int = Depends(get_current_user),
                         db: AsyncSession = Depends(get_db)):
...
async def send_message(
        ...
        chat_id: int | None | str = Form(None),
        user2_id: int | None | str = Form(None),
        files: list[UploadFile] = File(default=[]),
        user_id: int = Depends(get_current_user),
        db: AsyncSession = Depends(get_db)
):
    if isinstance(chat_id, str):
        try:                        #Костыль из-за несовершенностей swagger
            chat_id = int(chat_id)
        except Exception:
            if len(chat_id) == 0:
                chat_id = None
                ...
                raise HTTPException(
                    ...
                    detail="Incorrect type of chat_id"
                )
    if isinstance(user2_id, str):
        try:                        #Костыль из-за несовершенностей swagger
            ...
            if len(user2_id) == 0:
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Incorrect type of user_id"
                )
    if chat_id is not None and chat_id < 1:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="chat_id must be >= 1")
    if user2_id is not None and user2_id < 1:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="user2_id must be >= 1")
    if (not chat_id and not user2_id) or (chat_id and user2_id):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Specify either chat_id or user2_id"
        )
    ...
    new_message = MessageModel(user_id=user_id, chat_id=target_chat_id, text=text)
    db.add(new_message)
    await db.flush()
